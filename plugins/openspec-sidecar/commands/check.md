...
2. No file matching `openspec/specs/*-design.md` for default/openspec layouts. For `superpowers` layout, treat pre-existing `*-design.md` files as warnings (legacy superpowers snapshots) rather than errors; new work should use `*-spec.md`.
...
9. Every `*.md` under `openspec/specs` (excluding `TEMPLATE.md`) contains the heading `## ADR 级别决策识别`. For `superpowers` layout, files matching `*-design.md` are legacy snapshots and exempt from this requirement (warning only).
