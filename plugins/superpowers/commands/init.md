...
If the resolved layout is not the one this command was rendered for (paths in this prompt assume `docs/superpowers/design.md`), tell the user to re-install the matching layout's plugin instead of trying to remap paths at runtime.
...
3. For each of the four target files below: if it exists, skip and report; if missing and not `--dry-run`, create it with the embedded content. If `--dry-run`, report each missing file as would-create and do not write it. Use `Write` (do not modify existing files in this step).
   - `docs/superpowers/design.md` ← embedded section "design.md template"
   - `docs/superpowers/decisions/README.md` ← embedded section "decisions README template"
   - `docs/superpowers/decisions/TEMPLATE.md` ← embedded section "ADR template"
   - `docs/superpowers/specs/TEMPLATE.md` ← embedded section "spec template"
...
6. Output a structured report:
   ...
   - Next steps: read `docs/superpowers/design.md`, then run `/specguard:check`.
...
1. `docs/superpowers/design.md` 是当前架构唯一真相；写代码前必须先读它。
2. 决策档案在 `docs/superpowers/decisions/`；写新 ADR 前必须先查看既有 ADR。
3. 禁止新建 dated design 文件（如 `*-design.md`）；新切片写 `docs/superpowers/specs/<topic>-spec.md`，架构变更直接更新 design.md。
4. ADR 只用于五类硬条件：接口语义、数据格式、跨模块依赖、外部依赖、推翻既有设计；其他默认不写。
5. 接口/数据结构/模块边界变更必须同步 `docs/superpowers/design.md`；命中硬条件时还要写 ADR。
...
- 接口、数据结构、模块边界变化 → 必须同步 `docs/superpowers/design.md` 对应段落（默认动作，不询问）。
- 编辑 `docs/superpowers/design.md` 后更新顶部 `Last verified against code` 字段为当前 commit hash。
- 不允许在 `docs/superpowers/design.md` 中保留与代码不一致的陈述。
- mermaid 图与文字描述若不一致，文字为准，同步修图。
- 写 ADR 后，在 `docs/superpowers/design.md` 对应章节末尾追加 `（见 ADR-NNNN）`，并在 `docs/superpowers/decisions/README.md` 索引表加一行。
- **减法纪律**（每次修改 `docs/superpowers/design.md` 前扫一遍，发现以下内容必须移走或删除；见 ADR-0010）：
...
- 禁止新建 `docs/superpowers/specs/*-design.md`。新切片用 `*-spec.md`。
- spec 必须包含 `## ADR 级别决策识别` 节。
- 进入 plan 之前必须由用户拍板候选 ADR。
- 实施过程中接口/数据/边界变更必须同步 `docs/superpowers/design.md`。
```
        ...
        "hooks": [
          {
            ...
            "command": "f=$(python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get(\"tool_input\",{}).get(\"file_path\",\"\"))'); case \"$f\" in *docs/superpowers/specs/*-design.md) printf '%s\\n' '{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"deny\",\"permissionDecisionReason\":\"specguard: dated design files are forbidden. Update docs/superpowers/design.md or write docs/superpowers/specs/<topic>-spec.md instead.\"}}}' ;; *) printf '%s\\n' '{}' ;; esac"
          },
          {
            "type": "command",
            "statusMessage": "specguard: validate adr filename",
            "command": "f=$(python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get(\"tool_input\",{}).get(\"file_path\",\"\"))'); case \"$f\" in *docs/superpowers/decisions/*.md) base=$(basename \"$f\"); if echo \"$base\" | grep -qE '^[0-9]{4}-[a-z0-9-]+\\.md$|^TEMPLATE\\.md$|^README\\.md$'; then printf '%s\\n' '{}'; else printf '%s\\n' '{\"hookSpecificOutput\":{\"hookEventName\":\"PreToolUse\",\"permissionDecision\":\"deny\",\"permissionDecisionReason\":\"specguard: ADR filename must be NNNN-kebab-case.md\"}}}'; fi ;; *) printf '%s\\n' '{}' ;; esac"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            ...
            "command": "cd \"$CLAUDE_PROJECT_DIR\" 2>/dev/null || exit 0; src_changed=$(git diff --name-only HEAD 2>/dev/null | grep -c '^src/' || true); doc_changed=$(git diff --name-only HEAD 2>/dev/null | grep -cE '^(docs/superpowers/design\\.md|docs/superpowers/decisions/)' || true); if [ \"$src_changed\" -gt 0 ] && [ \"$doc_changed\" -eq 0 ]; then printf '%s\\n' '{\"systemMessage\":\"specguard: src/ changed but neither design.md nor decisions/ touched.\"}'; else printf '%s\\n' '{}'; fi"
          }
        ]
      }
    ],
